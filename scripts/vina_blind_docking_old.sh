...
# 专用Vina盲对接脚本
# 直接使用AutoDock Vina处理大尺寸盲对接
# 跳过AutoDock-GPU的网格限制

set -e

echo "=========================================="
echo "AutoDock Vina 盲对接流水线"
echo "专为大尺寸盲对接优化"
echo "=========================================="
...
# 定义文件
RECEPTOR_PDB="egfr.pdb"
LIGAND_PDB="Afatinib.pdb"
CONFIG_FILE="config.txt"
OUTPUT_PDBQT="results/Afatinib_out.pdbqt"

# 中间文件
RECEPTOR_PDBQT="egfr.pdbqt"
LIGAND_PDBQT="Afatinib.pdbqt"
...
if "$VINA_PATH" \
    ...
    --num_modes "$NUM_MODES" \
    --cpu 0 \
    > "logs/vina_blind_docking.log" 2>&1; then
    ...
    if [[ -f "$OUTPUT_PDBQT" ]] && [[ -s "$OUTPUT_PDBQT" ]]; then
        ...
        echo "详细日志: logs/vina_blind_docking.log"
        echo ""
        echo "🔍 下一步建议:"
        ...
        echo ""
        echo "检查Vina日志:"
        cat "logs/vina_blind_docking.log" | tail -20
    fi
    ...
    echo ""
    echo "错误信息:"
    cat "logs/vina_blind_docking.log" | tail -20 || echo "  无日志文件"
    echo ""
    echo "调试信息:"
